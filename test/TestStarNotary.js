...
it("can add the star name and star symbol properly", async () => {
  ...
  assert.equal(name, "Druk Star Token");
...
it("lets 2 users exchange stars", async () => {
  ...
  await instance.createStar("some awesome star", tokenId2, {
    from: user2,
  });
...
it("lookUptokenIdToStarInfo test", async () => {
  ...
  const tokenId = 10;
  let user = accounts[1];
  const starName = "FINAL_STAR";
  await instance.createStar(starName, tokenId, { from: user });
  // 2. Call your method lookUptokenIdToStarInfo
  const starNameFromToken = await instance.lookUptokenIdToStarInfo(tokenId);
  // 3. Verify if you Star name is the same
  assert.equal(starNameFromToken, starName);
});
